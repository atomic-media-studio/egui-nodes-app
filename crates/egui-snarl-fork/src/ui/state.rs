...
impl SnarlState {
    ...
    fn new_wires_in_mut(&mut self) -> Option<&mut SmallVec<[InPinId; 4]>> {
        match &mut self.new_wires {
            Some(NewWires::In(pins)) if !self.new_wires_menu => Some(pins),
            _ => None,
        }
    }

    fn new_wires_out_mut(&mut self) -> Option<&mut SmallVec<[OutPinId; 4]>> {
        match &mut self.new_wires {
            Some(NewWires::Out(pins)) if !self.new_wires_menu => Some(pins),
            _ => None,
        }
    }

    pub fn add_new_wire_in(&mut self, pin: InPinId) {
        let Some(pins) = self.new_wires_in_mut() else {
            debug_assert!(
                false,
                "add_new_wire_in called without active input wire bundle"
            );
            return;
        };
    ...
    pub fn add_new_wire_out(&mut self, pin: OutPinId) {
        let Some(pins) = self.new_wires_out_mut() else {
            debug_assert!(
                false,
                "add_new_wire_out called without active output wire bundle"
            );
            return;
        };
    ...
    pub fn remove_new_wire_in(&mut self, pin: InPinId) {
        let Some(pins) = self.new_wires_in_mut() else {
            debug_assert!(
                false,
                "remove_new_wire_in called without active input wire bundle"
            );
            return;
        };
    ...
    pub fn remove_new_wire_out(&mut self, pin: OutPinId) {
        let Some(pins) = self.new_wires_out_mut() else {
            debug_assert!(
                false,
                "remove_new_wire_out called without active output wire bundle"
            );
            return;
        };
