...
#[inline]
fn fallback_bezier5(frame_size: f32, from: Pos2, to: Pos2) -> [Pos2; 6] {
    let from_2 = pos2(from.x + frame_size, from.y);
    let to_2 = pos2(to.x - frame_size, to.y);
    let middle_1 = from_2.lerp(to_2, 1.0 / 3.0);
    let middle_2 = from_2.lerp(to_2, 2.0 / 3.0);
...
fn wire_bezier_5(frame_size: f32, from: Pos2, to: Pos2) -> [Pos2; 6] {
    ...
    if from_2.x <= to_2.x && between >= frame_size * 2.0 {
        ...
        if from_2.y >= to_2.y + frame_size {
            ...
            debug_assert!(false, "unreachable branch in wire_bezier_5 (case A)");
            return fallback_bezier5(frame_size, from, to);
        }
        ...
        debug_assert!(false, "unreachable branch in wire_bezier_5 (case B)");
        fallback_bezier5(frame_size, from, to)
    }
}
