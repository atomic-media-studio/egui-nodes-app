...
[ ] `graph-lib`: add `PinType` (or equivalent) on `Pin` and enforce compatibility in `Graph::connect` (propagate through `NodesEditor` / canvas so invalid connections are rejected early).

[ ] `graph-lib` `eval.rs`: extend `Value` for a Max-style message model — `Bang`, `Symbol`, `List`, plus numeric/bool as today; optional shared `Buffer` (e.g. `Arc<[f32]>`) only when you need block/sample data — not required until you touch DSP.
